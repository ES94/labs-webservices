		...
		if valid_json_data:
			...
			if validate_arguments['status'] == 'success':
				...
				SELECT date_format(en.fecha, '%Y/%m'), count(en.numero)
					...
					GROUP BY str_to_date(en.fecha, '%Y/%m');
				""".format(
					date_since,
					date_until,
					"AND id_medio = {}".format(id_media) if id_media else ""
			...
			if validate_arguments['status'] == 'success':
				...
				for form in forms_list:
					...
					SELECT '{}' formulario,
						...
						FROM {} f
						WHERE str_to_date(f.fecha_inicio, '%Y/%m/%d') BETWEEN str_to_date('{}', '%Y/%m/%d') AND str_to_date('{}', '%Y/%m/%d')
							AND f.fecha_fin = '' OR f.fecha_fin IS NULL
						GROUP BY str_to_date(f.fecha_inicio, '%Y/%m');
